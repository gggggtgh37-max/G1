...
# ── SESSION (single shared, no threading on Vercel) ───────────────────────── #
_session = None
def get_session():
    global _session
    if _session is None:
        ...
        ret = Retry(total=1, backoff_factor=0.3, status_forcelist=[429,500,502,503,504])
...
def make_proto(fields):
    ...
    for f,v in fields.items():
        if isinstance(v,dict):
            ...
            pkt.extend(enc_vr((f<<3)|0)+enc_vr(v))
    ...
    try:
        def _parse(results):
            ...
            for r in results:
                ...
                elif r.wire_type=="length_delimited": fd["data"]=_parse(r.data.results)
                d[r.field]=fd
            return d
        return json.loads(json.dumps(_parse(ProtoParser().parse(hex_str))))
    except: return None
...
def aes_enc(h):
    ...
    return c.encrypt(pad(bytes.fromhex(h),AES.block_size)).hex()
...
# ════════════════════════════════════════════════════════
#  MAIN FLOW — returns (account_dict | None, trace_dict)
# ════════════════════════════════════════════════════════
def create_acc(region, name_prefix):
    T   = {}          # trace dict — always populated
    ses = get_session()
    pwd = rand_pass()

    # ── S1: REGISTER ──────────────────────────────── #
    try:
        data = f"password={pwd}&client_type=2&source=2&app_id=100067"
        sig  = hmac.new(key, data.encode(), hashlib.sha256).hexdigest()
        r = ses.post(
            "https://100067.connect.garena.com/oauth/guest/register",
            headers={"User-Agent":"GarenaMSDK/4.0.19P8(ASUS_Z01QD ;Android 12;en;US;)",
        ...
        try: rj=r.json()
        except: rj={"raw":r.text[:300]}
        T["s1_register"]={"status":r.status_code,"body":rj}
        uid=rj.get("uid")
        if not uid:
            T["fail"]="s1_register: no uid"; return None,T
    except Exception as e:
        T["s1_register"]={"exception":str(e),"traceback":traceback.format_exc()[-500:]}
        T["fail"]="s1_register: exception"; return None,T

    # ── S2: TOKEN — try 3 client_secret variants ──── #
    secrets = [("latin1", key_str), ("hex_key", hex_key), ("app_id", "100067")]
    open_id=access_token=None
    for label, cs in secrets:
        try:
            r2=ses.post(
            ...
            try: rj2=r2.json()
            except: rj2={"raw":r2.text[:300]}
            T[f"s2_token_{label}"]={"status":r2.status_code,"body":rj2}
            if rj2.get("open_id") and rj2.get("access_token"):
                open_id=rj2["open_id"]; access_token=rj2["access_token"]
                T["s2_success_secret"]=label; break
        except Exception as e:
            T[f"s2_token_{label}_err"]={"exception":str(e),"tb":traceback.format_exc()[-300:]}

    if not open_id:
        T["fail"]="s2_token: all secrets failed"; return None,T
    ...
    try:
        ...
        pfields  = {1:name,2:access_token,3:open_id,5:102000007,6:4,7:1,13:1,14:field,15:"en",16:1,17:1}
        body     = bytes.fromhex(aes_enc(make_proto(pfields).hex()))
        r3=ses.post(
        ...
        T["fail"]="s3_major_reg: exception"; return None,T
    ...
    try:
        ...
        if r4.status_code==200 and len(r4.content)>=10:
            ...
            T["s4_login"]["jwt_found"]=bool(jwt)
            T["s4_login"]["jwt_prefix"]=jwt[:40] if jwt else None
    except Exception as e:
        T["s4_login"]={"exception":str(e),"tb":traceback.format_exc()[-300:]}
        T["fail"]="s4_login: exception"; return None,T

    # non-ar/en: ChooseRegion
    if lang not in ("ar","en") and jwt:
        try:
            rk="RU" if region=="RU" else region
            cr=bytes.fromhex(aes_enc(make_proto({1:rk}).hex()))
            rc=ses.post("https://loginbp.ggblueshark.com/ChooseRegion",
                headers={"User-Agent":"Dalvik/2.1.0 (Linux; U; Android 12; M2101K7AG Build/SKQ1.210908.001)",
                         "Connection":"Keep-Alive","Accept-Encoding":"gzip",
                         "Content-Type":"application/x-www-form-urlencoded","Expect":"100-continue",
                         "Authorization":f"Bearer {jwt}","X-Unity-Version":"2018.4.11f1",
                         "X-GA":"v1 1","ReleaseVersion":"OB52"},
                data=cr, verify=False, timeout=15)
            T["s4b_choose_region"]={"status":rc.status_code,"hex":rc.content.hex()[:80]}
            if rc.status_code==200:
                raw2=build_login_pl(access_token,open_id,lang)
                enc2=bytes.fromhex(aes_enc(raw2.hex()))
                ...
                T["s4c_login2"]["jwt_found"]=bool(jwt)
        except Exception as e:
            T["s4b_choose_region"]={"exception":str(e)}

    if not jwt:
        T["fail"]="s4_login: no JWT in response"; return None,T
    ...
    try:
        ...
        r5=ses.post(f"{link}GetLoginData",
            headers={"Expect":"100-continue","Authorization":f"Bearer {jwt}",
            ...
            data=enc5, verify=False, timeout=20)
        ...
        T["s5_getlogindata"]={"exception":str(e),"tb":traceback.format_exc()[-300:]}
        # non-fatal — account is created, just login data step failed
...
def generate_accounts():
    ...
    for i in range(count*5):
        ...
        else:       all_traces.append({"attempt":i+1,"failed_at":trace.get("fail"),"trace":trace})
        if len(results)<count: time.sleep(1)

    return jsonify({
        "success":True,"total_requested":count,
        "total_created":len(results),"accounts":results,
        "debug_failures": all_traces[:3] if not results else []
    })

@app.route("/trace", methods=["GET"])
def trace_single():
    """One attempt, full trace — USE THIS TO FIND THE BUG."""
    region=request.args.get("region","IND").upper()
...
def check_imports():
    """Check all imports work on this Vercel instance."""
    status={}
    try: import hmac; status["hmac"]="ok"
    except Exception as e: status["hmac"]=str(e)
    try: from Crypto.Cipher import AES; status["pycryptodome"]="ok"
    except Exception as e: status["pycryptodome"]=str(e)
    try: from protobuf_decoder.protobuf_decoder import Parser; status["protobuf_decoder"]="ok"
    except Exception as e: status["protobuf_decoder"]=str(e)
    try: import requests; status["requests"]="ok"
    except Exception as e: status["requests"]=str(e)
    status["proto_ok_at_startup"]=PROTO_OK
    if not PROTO_OK: status["proto_err"]=_proto_err_msg
    return jsonify(status)

@app.route("/ping-garena", methods=["GET"])
def ping_garena():
    """Test raw connectivity to Garena register endpoint."""
    try:
        r=requests.get("https://100067.connect.garena.com/",timeout=8)
...
def home():
    return jsonify({
        "endpoints":{
            "/trace":"?region=IND — FULL step trace, paste response here",
            "/imports":"check if all packages loaded",
            "/ping-garena":"test network to garena",
            "/gen":"?name=X&count=N&region=R",
        },"regions":list(REGION_LANG.keys())
    })

@app.route("/health")
def health(): return jsonify({"status":"healthy","proto_ok":PROTO_OK})
